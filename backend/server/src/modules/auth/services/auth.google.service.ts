...
export class GoogleAuthService {
  ...
  constructor() {
    ...
    this.ALLOWED_ADMIN_EMAILS = (env.ADMIN_EMAILS || '')
    .split(',')
  ...
  async authenticateWithGoogle(
    ...
    try {
      ...
      // Block admin emails from using Google sign-in entirely
      if (this.ALLOWED_ADMIN_EMAILS.includes(google_user.email.toLowerCase())) {
        logger.warn('Admin email attempted Google sign-in', { 
          email: google_user.email,
          ip: device_context.ip_address 
        });
        await AuditService.logSuspiciousActivity(
          undefined,
          'Admin email attempted Google OAuth sign-in',
          {
            ip_address: device_context.ip_address,
            user_agent: device_context.user_agent,
            risk_factors: ['admin_google_signin_attempt'],
            attempted_email: google_user.email
          }
        );
        return {
          success: false,
          error: 'Admin accounts must use the dedicated admin login with 2FA.',
          error_code: 'ADMIN_USE_DEDICATED_LOGIN'
        };
      }
  ...
  private async handleExistingUser(
    ...
    if (has_local) {
      ...
      return {
        ...
        error_code: 'ACCOUNT_EXISTS_LINK_REQUIRED',
        user
      };
    }
  ...
  async linkGoogleWithPassword(
    ...
    try {
      ...
      // Block admin emails here too
      if (this.ALLOWED_ADMIN_EMAILS.includes(google_user.email.toLowerCase())) {
        return {
          success: false,
          error: 'Admin accounts cannot use Google sign-in.',
          error_code: 'ADMIN_USE_DEDICATED_LOGIN'
        };
      }
