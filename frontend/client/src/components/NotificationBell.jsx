...
const NotificationBell = () => {
  ...
  return (
    <div className="relative">
      <button onClick={toggle} className="relative p-2 text-white hover:text-accent transition-colors">
        <Bell className="h-6 w-6" />
        {unread > 0 && (
          <span className="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 min-w-[18px] h-[18px] text-[10px] font-bold leading-none text-primary-950 bg-accent rounded-full transform translate-x-1/4 -translate-y-1/4">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
